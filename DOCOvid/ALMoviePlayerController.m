...
- (void)videoLoadStateChanged:(NSNotification *)note {
    switch (self.loadState) {
        case MPMovieLoadStatePlayable:
            ...
            if ([self.delegate respondsToSelector:@selector(moviePrepareToPlay)]) {
                [self.delegate performSelector:@selector(moviePrepareToPlay)];
            }
        default:
            break;
    }
...
-(void)movieTimedOut {
    [[NSNotificationCenter defaultCenter] postNotificationName:MPMoviePlayerLoadStateDidChangeNotification object:Nil];
...
{
    [self stop];
    NSTimeInterval readTime = [self currentPlaybackTime];
    [self setContentURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/ipad_high",_urlString]] hidden:NO];
    [self setInitialPlaybackTime:readTime];
    [self play];
}
...
- (void)playLowVedio
{
    [self stop];
    NSTimeInterval readTime = [self currentPlaybackTime];
    [self setContentURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/ipad_low",_urlString]] hidden:NO];
    [self setInitialPlaybackTime:readTime];
    [self play];
}
@end
