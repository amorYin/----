...
/*test*/
@property (nonatomic, strong) ALButton *alertButton;
@property (nonatomic, strong) ALButton *highButton;
@property (nonatomic, strong) ALButton *lowButton;
/*reset*/
...
- (id)initWithMoviePlayer:(ALMoviePlayerController *)moviePlayer style:(ALMoviePlayerControlsStyle)style {
    ...
    if (self) {
        ...
        _topBarHeight = [UIDevice iOSVersion] >= 7.0 ? 70.f : 50.f;
        _bottomBarHeight = 90.f;
...
- (void)volumeChanged:(NSNotification *)notification
{
    ...
    if (volume==0.) {
        _alertButton.selected = YES;
    }else{
        _alertButton.selected = NO;
    }
...
- (void)setup {
    ...
    if ([UIDevice iOSVersion]<7.0) {
        _topBar.color = [UIColor colorWithPatternImage:[UIImage imageNamed:@"navigationbars"]];
    }else{
        _topBar.color = [UIColor colorWithPatternImage:[UIImage imageNamed:@"navigationbar"]];
    }
    _topBar.alpha = 0.f;
    ...
    _bottomBar.color = [UIColor colorWithPatternImage:[UIImage imageNamed:@"vedio_bg_str"]];
    _bottomBar.alpha = 0.f;
    [self addSubview:_bottomBar];
    
    _durationSlider = [[UISlider alloc] init];
    _durationSlider.value = 0.f;
    [_durationSlider setMaximumTrackImage:[[UIImage imageNamed:@"time_senlder_str_n"] stretchableImageWithLeftCapWidth:3 topCapHeight:3] forState:UIControlStateNormal];
    [_durationSlider setMinimumTrackImage:[[UIImage imageNamed:@"time_senlder_str_h"] stretchableImageWithLeftCapWidth:3 topCapHeight:3] forState:UIControlStateNormal];
    [_durationSlider setThumbImage:[UIImage imageNamed:@"time_slender_p"] forState:UIControlStateNormal];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        [_fullscreenButton setImage:[UIImage imageNamed:@"back_btn_n"] forState:UIControlStateNormal];
        [_fullscreenButton setImage:[UIImage imageNamed:@"back_btn_h"] forState:UIControlStateHighlighted];
        ...
        [_scaleButton setImage:[UIImage imageNamed:@"open_btn_n"] forState:UIControlStateNormal];
        [_scaleButton setImage:[UIImage imageNamed:@"export_btn_h"] forState:UIControlStateHighlighted];
        [_scaleButton setImage:[UIImage imageNamed:@"export_btn_h"] forState:UIControlStateSelected];
        ...
        /*test*/
        _alertButton = [[ALButton alloc] init];
        ...
        [_collectButton setImage:[UIImage imageNamed:@"v_collect_btn_n"] forState:UIControlStateNormal];
        [_collectButton setImage:[UIImage imageNamed:@"v_collect_btn_h"] forState:UIControlStateSelected];
        [_collectButton addTarget:self action:@selector(collectVedio:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_collectButton];
        
        _shareButton = [[ALButton alloc] init];
        [_shareButton setImage:[UIImage imageNamed:@"v_share_btn_n"] forState:UIControlStateNormal];
        [_shareButton setImage:[UIImage imageNamed:@"v_share_btn_h"] forState:UIControlStateSelected];
        [_shareButton addTarget:self action:@selector(shareVedio:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_shareButton];
        
        _downButton = [[ALButton alloc] init];
        [_downButton setImage:[UIImage imageNamed:@"v_down_btn_n"] forState:UIControlStateNormal];
        [_downButton setImage:[UIImage imageNamed:@"v_down_btn_h"] forState:UIControlStateSelected];
        [_downButton addTarget:self action:@selector(downVedio:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_downButton];
        
        
        _highButton = [[ALButton alloc] init];
        [_highButton setImage:[UIImage imageNamed:@"hd_btn_n"] forState:UIControlStateNormal];
        [_highButton setImage:[UIImage imageNamed:@"hd_btn_h"] forState:UIControlStateHighlighted];
        [_highButton setImage:[UIImage imageNamed:@"hd_btn_h"] forState:UIControlStateSelected];
        [_highButton addTarget:self action:@selector(playHightVedio:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_highButton];
        
        _lowButton = [[ALButton alloc] init];
        [_lowButton setImage:[UIImage imageNamed:@"nomal_btn_n"] forState:UIControlStateNormal];
        [_lowButton setImage:[UIImage imageNamed:@"nomal_btn_h"] forState:UIControlStateHighlighted];
        [_lowButton setImage:[UIImage imageNamed:@"nomal_btn_h"] forState:UIControlStateSelected];
        [_lowButton addTarget:self action:@selector(playLowVedio:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_lowButton];
        _lowButton.selected = YES;
        
        _volumeView = [[MPVolumeView alloc] init];
        [_volumeView setShowsRouteButton:YES];
        [_volumeView setShowsVolumeSlider:YES];
        [_volumeView setMinimumVolumeSliderImage:[[UIImage imageNamed:@"volum_str_h"] stretchableImageWithLeftCapWidth:7 topCapHeight:7] forState:UIControlStateNormal];
        [_volumeView setMaximumVolumeSliderImage:[[UIImage imageNamed:@"volum_str_n"] stretchableImageWithLeftCapWidth:7 topCapHeight:7] forState:UIControlStateNormal];
        [_volumeView setVolumeThumbImage:[UIImage imageNamed:@"volum_p"] forState:UIControlStateNormal];
    ...
    [_playPauseButton setImage:[UIImage imageNamed:@"v_pause_btn_n"] forState:UIControlStateNormal];
    [_playPauseButton setImage:[UIImage imageNamed:@"v_play_btn_n"] forState:UIControlStateSelected];
...
- (void)setState:(ALMoviePlayerControlsState)state {
    if (_state != state) {
        ...
        switch (state) {
            ...
            case ALMoviePlayerControlsStateError:
...
- (void) playHightVedio:(ALButton*)btn
{
    _highButton.selected = YES;
    _lowButton.selected = NO;
    [self.moviePlayer playHighVedio];
}

- (void) playLowVedio:(ALButton*)btn
{
    _highButton.selected = NO;
    _lowButton.selected = YES;
    [self.moviePlayer playLowVedio];
}
...
- (void)movieLoadStateDidChange:(NSNotification *)note {
    switch (self.moviePlayer.loadState) {
        ...
        case MPMovieLoadStateUnknown:
            [self showControls:nil];
            [self stopDurationTimer];
            self.state = ALMoviePlayerControlsStateError;
        default:
            break;
    }
}
...
- (void)layoutSubviews {
    ...
    CGFloat paddingBetweenLabelsAndSlider = 110.f;
    CGFloat sliderHeight = 40.f; //default height
...
//    CGFloat volumeWidth = isIpad() ? 210.f : 120.f;
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        //top bar
        CGFloat fullscreenWidth = 48.f;
        CGFloat hdbuttonWidth = 60.f;
        CGFloat fullscreenHeight = self.bottomBarHeight;
        CGFloat scaleWidth = 40.f;
        CGFloat scaleHeight = 40.f;
        self.topBar.frame = CGRectMake(0, 0, self.frame.size.width, self.topBarHeight);
        ...
        //bottom bar
        self.bottomBar.frame = CGRectMake(0, self.frame.size.height - self.bottomBarHeight, self.frame.size.width, self.bottomBarHeight);
        self.playPauseButton.frame = CGRectMake(self.bottomBar.frame.size.width/2 - playWidth/2, self.bottomBarHeight/2 - playHeight/2, playWidth, playHeight);
        self.seekForwardButton.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width + paddingBetweenPlaybackButtons, self.bottomBarHeight/2 - seekHeight/2 + 1.f, seekWidth, seekHeight);
        self.seekBackwardButton.frame = CGRectMake(self.playPauseButton.frame.origin.x - paddingBetweenPlaybackButtons - seekWidth, self.bottomBarHeight/2 - seekHeight/2 + 1.f, seekWidth, seekHeight);
...
//            self.alertButton.alpha = 0.f;
//        } else {
//            self.volumeView.alpha = 1.f;
//            self.volumeView.frame = CGRectMake(paddingFromBezel+volumeHeight+5, self.bottomBarHeight/2 - volumeHeight/2, volumeWidth, volumeHeight);
//        }
        
        self.airplayView.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - airplayWidth, self.bottomBarHeight/2 - airplayHeight/2, airplayWidth, airplayHeight);
        
        self.collectButton.frame = CGRectMake(paddingFromBezel,  self.bottomBarHeight/2 - fullscreenWidth/2, fullscreenWidth, fullscreenWidth);
        
        self.shareButton.frame = CGRectMake(CGRectGetMaxX(self.collectButton.frame)+paddingFromBezel,self.bottomBarHeight/2 - fullscreenWidth/2, fullscreenWidth, fullscreenWidth);
        
        self.downButton.frame = CGRectMake(CGRectGetMaxX(self.shareButton.frame)+paddingFromBezel+10,self.bottomBarHeight/2 - fullscreenWidth/2, fullscreenWidth, fullscreenWidth);
        
        
        self.lowButton.frame = CGRectMake(CGRectGetMaxX(self.downButton.frame)+paddingFromBezel+10, self.bottomBarHeight/2 - scaleHeight/2, hdbuttonWidth, scaleHeight);
        ...
        self.volumeView.alpha = 1.f;
        self.volumeView.frame = CGRectMake(CGRectGetMaxX(self.playPauseButton.frame)+paddingBetweenLabelsAndSlider, self.bottomBarHeight/2 - volumeHeight/2, CGRectGetMinX(self.alertButton.frame)-CGRectGetMaxX(self.playPauseButton.frame)-paddingBetweenLabelsAndSlider, volumeHeight);
    }
...
//        self.playPauseButton.frame = CGRectMake(paddingFromBezel, self.bottomBarHeight/2 - playHeight/2, playWidth, playHeight);
//        self.timeElapsedLabel.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.bottomBarHeight);
//        
//        //right side of bottom bar
//        CGFloat fullscreenWidth = 28.f;
//        CGFloat fullscreenHeight = fullscreenWidth;
//        self.fullscreenButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - fullscreenWidth, self.bottomBarHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
//        self.airplayView.frame = CGRectMake(self.fullscreenButton.frame.origin.x - paddingBetweenButtons - airplayWidth, self.bottomBarHeight/2 - airplayHeight/2, airplayWidth, airplayHeight);
//        self.timeRemainingLabel.frame = CGRectMake(self.airplayView.frame.origin.x - paddingBetweenButtons - labelWidth, 0, labelWidth, self.bottomBarHeight);
    }
    ...
    self.durationSlider.frame = CGRectMake(timeElapsedX + self.timeElapsedLabel.frame.size.width + paddingBetweenLabelsAndSlider, self.bottomBarHeight/2 - sliderHeight/2, sliderWidth, sliderHeight);
    */
    
    self.durationSlider.frame = CGRectMake(-5 , -sliderHeight/2, self.width+10, sliderHeight);
